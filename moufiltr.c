...
--*/
{
    ...
    // If it's an absolute report, set virtual desktop flag
    PMOUSE_INPUT_DATA InputDataCurrent;
    for (InputDataCurrent = InputDataStart; InputDataCurrent < InputDataEnd; InputDataCurrent++) {

        if ((InputDataCurrent->Flags & MOUSE_MOVE_ABSOLUTE) == MOUSE_MOVE_ABSOLUTE) {
            InputDataCurrent->Flags |= MOUSE_VIRTUAL_DESKTOP;
        }
